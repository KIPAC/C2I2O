...
def test_scipy_wrap() -> None:
    ...
    for i, (dist_name, model) in enumerate(models.items()):
        ...
        for name, field in model.model_fields.items():
            ...
            print(f"- {name}: ({required_status})")

    # --- Example Usage: Validate the Beta Distribution ---
    if "beta" in models:
        beta_params = models["beta"]
        print("\n--- Example Validation: Beta Distribution (requires 'a' and 'b') ---")

        # 1. Valid data
        valid_data = {"a": 2.5, "b": 7.0, "loc": 10, "scale": 5}
        try:
            validated_beta = beta_params(**valid_data)
            print(
                f" Success: Validated Beta data: {validated_beta.model_dump_json(indent=2)}"
            )
    ...
    if "norm" in models:
        ...
        try:
            validated_norm = norm_params(**valid_norm_data)
            print(
                f" Success: Validated Normal data: {validated_norm.model_dump_json(indent=2)}"
            )
        except Exception as e:
            print(f" Error during valid validation: {e}")
