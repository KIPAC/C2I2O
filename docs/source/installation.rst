...
There are two ways you might choose to install `C2I2O`

1. `Production Installation`_: Just install `C2I2O` in an
   existing an existing conda environment using pip.
2. `Developer Installation`_: Download the `C2I2O` source
...
Here we will be installing ``C2I2O`` into an existing conda environment "[env]".
